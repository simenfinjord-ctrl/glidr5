...
import { X, ChevronRight, ChevronLeft, Zap, ClipboardList, Thermometer, BarChart3, Check } from "lucide-react";
  ...
  const STEPS = [
    {
      ...
      subtitle: t("onboarding.step.welcome.subtitle"),
      body: t("onboarding.step.welcome.body"),
      visual: (
        <div className="grid grid-cols-3 gap-2 w-full">
          {[
            { label: t("onboarding.step.welcome.tests"), value: "0", icon: "🎿", color: "bg-blue-50 dark:bg-blue-900/20 text-blue-600" },
            { label: t("onboarding.step.welcome.products"), value: "0", icon: "📦", color: "bg-violet-50 dark:bg-violet-900/20 text-violet-600" },
            { label: t("onboarding.step.welcome.seasons"), value: "0", icon: "🏔️", color: "bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600" },
          ].map((c) => (
    ...
    },
    {
      ...
      title: t("onboarding.step.products.title"),
      subtitle: t("onboarding.step.products.subtitle"),
      body: t("onboarding.step.products.body"),
      visual: (
        <div className="rounded-xl border bg-muted/30 p-3 text-xs space-y-1.5">
          ...
          <div className="flex items-center gap-2 rounded-lg border-2 border-dashed border-muted-foreground/25 px-3 py-2 text-muted-foreground/50">
            ...
            <span>{t("onboarding.step.products.addProduct")}</span>
          </div>
        </div>
      ),
    },
    {
      ...
      title: t("onboarding.step.weather.title"),
      subtitle: t("onboarding.step.weather.subtitle"),
      body: t("onboarding.step.weather.body"),
      visual: (
        <div className="rounded-xl border bg-card p-3 text-xs">
          <div className="font-semibold text-foreground mb-2.5">{t("onboarding.step.weather.newEntry")}</div>
    ...
    },
    {
      ...
      title: t("onboarding.step.tests.title"),
      subtitle: t("onboarding.step.tests.subtitle"),
      body: t("onboarding.step.tests.body"),
      visual: (
        <div className="rounded-xl border bg-card p-3 text-xs space-y-1.5">
          <div className="flex items-center gap-2 mb-2">
            <span className="font-semibold text-foreground">{t("onboarding.step.tests.glideTest")}</span>
    ...
    },
    {
      ...
      title: t("onboarding.step.analytics.title"),
      subtitle: t("onboarding.step.analytics.subtitle"),
      body: t("onboarding.step.analytics.body"),
      visual: (
        <div className="rounded-xl border bg-card p-3 text-xs">
          <div className="font-semibold text-foreground mb-3">{t("onboarding.step.analytics.overview")}</div>
          <div className="grid grid-cols-2 gap-2 mb-3">
            {[
              { label: t("onboarding.step.welcome.tests"), v: "0", icon: "📋" },
              { label: t("onboarding.step.welcome.products"), v: "0", icon: "📦" },
              { label: t("onboarding.step.analytics.wins"), v: "0", icon: "🏆" },
              { label: t("onboarding.step.analytics.withWeather"), v: "0%", icon: "🌡️" },
            ].map((s) => (
          ...
          <div className="rounded-lg bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200/50 px-3 py-2 text-emerald-700 dark:text-emerald-400 text-[11px]">
            {t("onboarding.step.analytics.hint")}
          </div>
        </div>
      ),
    },
    {
      ...
      subtitle: t("onboarding.step.done.subtitle"),
      body: t("onboarding.step.done.body"),
      visual: (
        <div className="flex flex-col items-center gap-3 py-2">
          ...
          <div className="text-sm font-semibold text-foreground">{t("onboarding.step.done.setupComplete")}</div>
          <div className="flex flex-col gap-2 w-full text-xs">
            {[
              { label: t("onboarding.step.done.addProducts"), href: "/products" },
              { label: t("onboarding.step.done.logWeather"), href: "/weather" },
              { label: t("onboarding.step.done.createTest"), href: "/tests/new" },
            ].map((a) => (
              <Link key={a.href} href={a.href} className="flex items-center gap-2 rounded-xl border bg-muted/40 px-4 py-2.5 font-medium text-foreground hover:bg-muted transition-colors">
